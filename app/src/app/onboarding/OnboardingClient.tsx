...
// ── TYPES ──────────────────────────────────────────────────────
type Screen = 'role' | 'quiz' | 'info' | 'success'
...
const CSS = `
  ...
  .upload-zone { border:2px dashed var(--border); border-radius:16px; padding:32px 20px; text-align:center; cursor:pointer; transition:all 0.2s; background:var(--card); position:relative; }
  .upload-zone:hover { border-color:var(--primary); background:var(--primary-soft); }
  .upload-zone.has-files { border-style:solid; border-color:var(--accent); background:var(--accent-soft); }
  .upload-icon .material-symbols-rounded { font-size:36px; color:var(--ink3); margin-bottom:10px; display:block; }
  ...
  .upload-previews { display:flex; flex-wrap:wrap; gap:10px; margin-top:16px; justify-content:center; }
  .upload-thumb { width:72px; height:72px; border-radius:10px; object-fit:cover; border:2px solid var(--accent-soft); }
  ...
  @keyframes successPulse { 0%{box-shadow:0 0 0 0 rgba(123,158,135,0.5);} 70%{box-shadow:0 0 0 18px rgba(123,158,135,0);} 100%{box-shadow:0 0 0 0 rgba(123,158,135,0);} }
  .success-icon.pulsing { animation:successPulse 2s ease-out infinite; }
...
function StreetAutocomplete({ city, value, onChange, hasError }: {
    ...
    timer.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `https://api.dataforsyningen.dk/vejnavne/autocomplete?q=${encodeURIComponent(q)}&kommunekode=${code}&per_side=8`
        )
  ...
  function handleBlur() {
    setTimeout(() => {
      setOpen(false)
      // Nulstil til senest bekræftede valg — tvinger brugeren til at vælge fra listen
      setQuery(confirmedValue.current)
    }, 150)
  }
...
// ── COMPONENT ──────────────────────────────────────────────────
function AvatarUpload({ preview, onChange }: { preview: string | null; onChange: (file: File, url: string) => void }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 20 }}>
      <label style={{ position: 'relative', cursor: 'pointer' }}>
        <div style={{ width: 88, height: 88, borderRadius: '50%', background: 'var(--accent-soft)', border: `3px ${preview ? 'solid var(--accent)' : 'dashed var(--accent)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', transition: 'border-color 0.2s' }}>
          {preview
            ? <img src={preview} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            : <span className="material-symbols-rounded" style={{ fontSize: 32, color: 'var(--accent)' }}>add_a_photo</span>
          }
        </div>
      ...
      <span style={{ fontSize: 12, color: 'var(--ink3)', marginTop: 8 }}>{preview ? 'Klik for at skifte' : 'Tilføj profilbillede (valgfrit)'}</span>
    </div>
  )
}
...
export default function OnboardingClient({ userId }: { userId: string }) {
  ...
  // Dev preview: læs initial state fra URL-params
  const initStep = params.get('step') as Screen | null
  const initQ    = parseInt(params.get('q') ?? '0', 10)
  const initRole = params.get('role') as Role | null
  ...
  const [screen, setScreen] = useState<Screen>(initStep ?? 'role')
  const [role, setRole] = useState<Role | null>(initRole)
  ...
  function goToSuccess() {
    setScreen('success')
    setTimeout(() => setSuccessPulse(true), 600)
    submitData()
  }
  ...
  function submitData() {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (role === 'udlejer' && uploadFiles.length > 0) imageUrls = await uploadListingImages(uploadFiles, userId)
      } catch (e) {
        setServerError(e instanceof Error ? e.message : 'Billedupload fejlede')
        return
      }
      const result = await saveOnboarding({ ...data, imageUrls, avatarUrl })
      if ('error' in result) {
          ...
          <div style={{ width: '100%', maxWidth: 560 }}>
            ...
            {screen === 'info' && role === 'udlejer' && (
              <div className="screen" key="info-udlejer">
                ...
                <AvatarUpload preview={avatarPreview} onChange={(f, url) => { setAvatarFile(f); setAvatarPreview(url) }} />
                ...
                <div className="form-grid">
                  ...
                  <div className="field-group">
                    <label className="field-label">Billeder af lejligheden</label>
                    <div className={'upload-zone' + (uploadFiles.length > 0 ? ' has-files' : '')}>
                      <input
                        type="file" className="upload-input" accept="image/*" multiple
                      ...
                      <div className="upload-sub">
                        {uploadFiles.length > 0 ? 'Klik for at ændre valg' : 'Træk og slip, eller klik for at vælge · Maks. 8 billeder'}
                      </div>
                      {uploadPreviews.length > 0 && (
                        <div className="upload-previews">
                          {uploadPreviews.map((src, i) => (
                            <img key={i} className="upload-thumb" src={src} alt="" />
                          ))}
                        </div>
                      )}
                    </div>
                ...
                <button className="btn-primary" onClick={validateUdlejer} disabled={isPending}>
                  Opret profil <span className="material-symbols-rounded">check</span>
                </button>
                {serverError && (
                  <p style={{ color: '#E05252', fontSize: 13, marginTop: 12, textAlign: 'center' }}>{serverError}</p>
                )}
              </div>
            )}
            ...
            {screen === 'info' && role === 'søgende' && (
              <div className="screen" key="info-søgende">
                ...
                <button className="btn-primary" onClick={validateSøgende} disabled={isPending}>
                  Opret profil <span className="material-symbols-rounded">check</span>
                </button>
                {serverError && (
                  <p style={{ color: '#E05252', fontSize: 13, marginTop: 12, textAlign: 'center' }}>{serverError}</p>
            ...
            {/* ── STEP 4: SUCCESS ── */}
            {screen === 'success' && (
              <div className="screen" key="success">
                <div className="card" style={{ textAlign: 'center' }}>
                  <div className={'success-icon' + (successPulse ? ' pulsing' : '')}>
                  ...
                  <div style={{ background: 'var(--bg)', borderRadius: 14, padding: '16px 18px', border: '1px solid var(--border)', marginBottom: 24, textAlign: 'left' }}>
                  ...
                  {serverError && (
                    <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, padding: '12px 16px', marginTop: 12, textAlign: 'left' }}>
                      <p style={{ color: '#E05252', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Der opstod en fejl:</p>
                      <p style={{ color: '#E05252', fontSize: 13 }}>{serverError}</p>
                      <button
                        onClick={() => { setServerError(null); submitData() }}
                        style={{ marginTop: 10, fontSize: 13, fontWeight: 700, color: 'var(--primary)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}
                      >
                        Prøv igen
                      </button>
                    </div>
