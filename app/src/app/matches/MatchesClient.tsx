            ...
            {list.length === 0 ? (
              <div className="empty-state">
                <span className="empty-icon">
                  {activeTab === 'favoritter' ? '⭐' : cards.length === 0 ? '🏘️' : '🔍'}
                </span>
                <div className="empty-title">
                  {activeTab === 'favoritter'
                    ? 'Ingen favoritter endnu'
                    : cards.length === 0
                      ? (userRole === 'seeker' ? 'Ingen lejligheder i din by endnu' : 'Ingen søgende i nærheden endnu')
                      : 'Ingen matches med dette filter'}
                </div>
                <div className="empty-sub">
                  {activeTab === 'favoritter'
                    ? 'Tryk på stjerne-ikonet på et kort for at gemme det.'
                    : cards.length === 0
                      ? 'Vi giver dig besked, når der dukker nogen op der matcher dig.'
